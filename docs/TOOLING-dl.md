...
> **Renamed 2026-08-13.** The binary is `dl`; the project and namespace remain
> `Deadlock.Patch`. Four modes: `set` (default, no subcommand), `batch`,
> `diff`, `pack`.
...
```
dl --in <file.vdata> --out <file.vdata> --set <dotted.path>=<value> [--set ...]
         [--dry-run] [--json]
```
...
## Envelope emission — one place, 2026-08-13

`CommandIo` owns envelope construction, the tool identity, the pinned build,
and the JSON-versus-stderr branch. Commands supply only their genuinely
different human output, via two callbacks:

| callback | when |
|---|---|
| `body` | per-item lines, BEFORE the error block |
| `footer` | closing summary, AFTER the error block |

Split that way to preserve the ordering every command already had — what
happened, then why it failed, then the summary. There is exactly one
`Json.ToStdout` in the codebase; adding a fifth mode must not add a second.

## Failure kinds are values, not prose — 2026-08-13

`EditResult.Failure` is an `EditFailure` enum: `None`, `PathNotFound`,
`NotAScalar`, `Flagged`, `TypeMismatch`, `Malformed`. Front ends branch on it
to choose an exit code.

Both front ends previously did `r.Error.Contains("path not found")`, which
contradicted the project's own principle — agents branch on codes, not prose —
and meant **rewording an error message silently changed exit-code behaviour**.
The message remains for humans and carries no behaviour.

## Measurement

`BatchData.peak_memory_mb` reports peak working set. Batch holds every parsed
document in memory and the 32-file cap was set by judgement; first reading is
**63 MB for 2 files**, one of them `heroes.vdata`, so the cap is conservative.
